...
def list_config_attributes(config_class: object, *ignore_args) -> List[str]:
    return [
        name
...
def set_config_option_from_class(config_mixin, args, config_data: dict, *ignore_args):
    for option in list_config_attributes(config_mixin, ignore_args):
...
def set_server_options(args, config_data: dict):
    set_config_option_from_class(ServerConfigMixin, args, config_data, 'jwt_secret_key')


def set_worker_options(args, config_data: dict):
    set_config_option_from_class(WorkerConfigMixin, args, config_data, 'resources')
