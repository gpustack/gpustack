...
def _numbered_tail_shards(head_log_path: Path) -> List[Tuple[int, Path]]:
    ...
    directory = head_log_path.parent
...
def _head_and_shard(segment_log_path: Path) -> Tuple[Path, int]:
...
def next_segment_log_path(segment_log_path: Path) -> Optional[Path]:
    ...
    """
    head, shard = _head_and_shard(segment_log_path)
    successor = tail_shard_log_path(head, shard + 1)
    return successor if successor.exists() else None
...
_MARKER_TEMPLATE = (
    ...
    "serving log size cap (GPUSTACK_SERVE_LOG_MAX_BYTES) ...\n"
)
_MARKER_RE = re.compile(r'^\.\.\. (?P<bytes>\d+) bytes / (?P<lines>\d+) lines omitted')
...
class _TextSinkBuffer:
    ...
    them and a rotation cannot leave them in a closed file. The decoder is
    incremental because a character may be split across two writes.
    """
    ...
    def write(self, data) -> int:
        self._sink.write(self._decoder.decode(bytes(data)))
        return len(data)
...
class CappedLogWriter(io.TextIOBase):
    ...
    def write(self, s: str) -> int:
        ...
        with self._lock:
            ...
            if self._capped:
                ...
                self._note_segment_size(self._segment_bytes + size)
            return written
    ...
    def _roll(self):
        ...
        self._file = self._open(
            tail_shard_log_path(self.head_path, self._shard), append=False
        )
        self._note_segment_size(0)
        self._drop_expired_shards()

    def _drop_expired_shards(self):
        shards = tail_shard_log_paths(self.head_path)
        dropped = False
        for path in shards[: max(0, len(shards) - self._max_shards)]:
            try:
                size = path.stat().st_size
                lines = _count_lines(path)
                path.unlink()
            except OSError as e:
                logger.warning(f"Failed to drop tail shard {path}: {e}")
                continue
            # Counted once the shard is gone: a shard that failed to go would
            # otherwise be counted again on the next roll.
    ...
    def _write_marker(self):
        try:
            marker_log_path(self.head_path).write_text(
            ...
            logger.warning(f"Failed to record the truncation of {self.head_path}: {e}")

    def _read_marker(self):
        # The totals live in the marker, not in memory, so a worker restart
        # keeps counting instead of starting from zero.
        try:
            text = marker_log_path(self.head_path).read_text(encoding='utf-8')
        except OSError:
            return
        match = _MARKER_RE.match(text)
        if match:
            self._omitted_bytes = int(match.group("bytes"))
            self._omitted_lines = int(match.group("lines"))
