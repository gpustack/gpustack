...
from typing import Annotated, Iterable, Iterator, List, Optional, Tuple

import aiofiles
from aiofiles.threadpool.text import AsyncTextIOWrapper
from fastapi import Depends, Query

from gpustack.worker.log_sources import next_segment_log_path
...
# One lock for all of them: a torn checkpoint list is worse than two threads
# waiting out one scan, and a scan is bounded by the serving log size cap.
...
def _extend_index(path: Path, index: LogFileIndex):
    with open(path, 'rb') as f:
        ...
        while True:
            ...
            position += len(block)
        index.size = position
        f.seek(max(0, position - _INDEX_FINGERPRINT_BYTES))
        index.tail = f.read(_INDEX_FINGERPRINT_BYTES)
...
def index_log_file(path: Path) -> LogFileIndex:
    ...
    with _index_lock:
        ...
        if index is None or index.file_id != file_id or size < index.size:
            index = LogFileIndex(file_id=file_id)
        try:
            if not _index_describes(path, index):
                index = LogFileIndex(file_id=file_id)
            if size > index.size:
...
class LineWindow:
    ...
    Attributes:
        ...
        line_count: How many lines it holds; fewer than asked for at the end.
        total_lines: Lines in the whole stream, as the index sees it now.
        reads: (path, first line within that file, how many lines) in order.
    """

    offset: int
    line_count: int
    total_lines: int
    reads: Tuple[Tuple[Path, int, int], ...]
...
def plan_line_window(paths: Iterable[Path], offset: int, limit: int) -> LineWindow:
    ...
    wanted = limit
    for path in paths:
        lines = index_log_file(path).line_count
        start = offset - total
        if wanted > 0 and start < lines:
            first = max(0, start)
            take = min(wanted, lines - first)
            if take > 0:
                reads.append((path, first, take))
                wanted -= take
        total += lines
    return LineWindow(
        offset=offset,
...
def read_line_slice(path: Path, first: int, count: int) -> Iterator[bytes]:
...
async def line_window_generator(window: LineWindow):
    ...
    for path, first, count in window.reads:
        chunks = read_line_slice(path, first, count)
        try:
            while True:
            ...
            # Cancelling the await abandons the read, it does not stop it, so
            # the reader may still be inside the generator here -- and closing
            # one mid-read raises, which would replace the cancellation with an
            # error. The abandoned read ends on its own a chunk later.
...
async def log_generator(path: str, options: LogOptions):
    ...
    while True:
        ...
        # is not the end of the log.
        successor = next_segment_log_path(Path(path)) if options.follow else None
...
async def follow_file(
    ...
    leaving the caller to continue in the next one. The writer closes a shard
    before creating its successor, so one more read takes the rest of it.
    """
    while True:
        ...
        if line:
            yield line
            continue
        if path is not None and next_segment_log_path(Path(path)) is not None:
