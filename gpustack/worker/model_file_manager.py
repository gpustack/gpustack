...
class ModelFileManager:
    ...
    async def _handle_deletion(self, model_file: ModelFile):
        ...
        if entry:
            ...
            try:
                ...
                logger.info(f"Cancelled download for deleted model: {model_file.id}")
    ...
    async def _delete_model_file(self, model_file: ModelFile):
        try:
            ...
            logger.info(f"Deleted model file {model_file.id}")
        except Exception as e:
            logger.debug(f"Failed to delete {model_file.id}: {e}")
    ...
    def _create_download_task(self, model_file: ModelFile):
        ...
        logger.debug(f"Created download task for {model_file.id}")
        ...
        async def _check_completion():
            ...
            logger.debug(f"Download completed for {model_file.id}")
...
class ModelFileDownloadTask:
    ...
    def _download_model_file(self):
        try:
            logger.info(f"Downloading {self._model_file.id}")
            ...
            logger.info(f"Successfully downloaded {self._model_file.id}")
