...
class ModelFileDownloadTask:
    ...
    def __init__(self, model_file: ModelFile, cfg: Config, cancel_flag):
        ...
        self._instance_download_log_files = []
    ...
    def _setup_instance_log_files(self):
        try:
            ...
            self._instance_download_log_files.append(str(download_log_file_path))
    ...
    def _write_to_instance_download_logs(self, message: str, is_error=False):
        ...
        """
        if not self._instance_download_log_files or self._download_completed:
            return
        ...
        for log_file_path in self._instance_download_log_files:
            try:
                # Ensure log directory exists
                Path(log_file_path).parent.mkdir(parents=True, exist_ok=True)

                # Use file locking to prevent concurrent writes from corrupting logs
                import fcntl
