...
class ServeManager:
    ...
    def _dp_member_should_wait(self, mi: ModelInstance, model: Model) -> bool:
        ...
        try:
            siblings = list_dp_siblings(self._clientset, mi.model_id)
        except Exception as e:
            # Keep waiting rather than let the caller's handler mark the
            # instance ERROR: a transient API failure says nothing about
            # whether the coordinator is up.
            logger.warning(
                f"Failed to list DP siblings of model {mi.model_id} for "
                f"{mi.name}: {e}. Retrying on the next reconcile."
            )
            return True

        coordinator = next(
            (sibling for sibling in siblings if sibling.dp_rank == 0),
            None,
        )
