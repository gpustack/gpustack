...
from gpustack.utils.vllm_topology import has_manual_distributed_params
...
async def validate_model_in(
    ...
    *,
    ...
    if model_in.backend_parameters:
        ...
        if has_manual_distributed_params(
            model_in.backend_parameters
        ) and not manual_distributed_from_env(model_in.env):
            raise BadRequestException(
                message=(
                    "Detected manually specified distributed parameters (e.g. "
                    "--data-parallel-address / --node-rank) without "
                    "GPUSTACK_MANUAL_DISTRIBUTED. To wire a data-parallel cluster "
                    "yourself, set GPUSTACK_MANUAL_DISTRIBUTED=true (with "
