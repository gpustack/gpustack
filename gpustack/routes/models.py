...
def apply_scaling_schedule_baseline(
    ...
    DP-node-per-instance models are exempt: their ``replicas`` is the DP node
    count fixed by the data-parallel topology, not a scaling knob.
    """
    schedule = getattr(model_in, "scaling_schedule", None)
    if not schedule or not schedule.enabled:
        return
    if is_dp_node_per_instance(model_in):
        return
