...
def _stream_stats(paths: List[Path]) -> ModelInstanceLogStreamStats:
    """Measure one stream, so the viewer knows its size before reading it.
    ...
    Returns:
        Total bytes, total lines, and whether the size cap dropped any of it.
    """
    size_bytes = 0
    line_count = 0
    truncated = False
    for path in paths:
        ...
        if parsed is not None and parsed.segment is ServeLogSegment.MARKER:
            truncated = True
    return ModelInstanceLogStreamStats(
...
def restart_entries_from_main_log_files(
    ...
    stats_by_restart: Optional[
        Dict[int, Dict[str, ModelInstanceLogStreamStats]]
    ] = None,
    ...
    Args:
        ...
        stats_by_restart: Mapping from restart_count to the size of each of
            its streams, by container name.
    """
    ...
    for i, rc in enumerate(sorted_counts):
        ...
        entries.append(
            ModelInstanceLogRestartEntry(
                ...
                container_stats=(stats_by_restart or {}).get(rc, {}),
            )
        )
    return entries
    ...
    async def read_to_queue(queue: asyncio.Queue, paths: List[str], opts: LogOptions):
        try:
            for index, log_path in enumerate(paths):
                ...
                file_options = LogOptions(
                    tail=opts.tail if is_last else -1,
                    follow=opts.follow and is_last,
...
async def get_serve_log_options(request: Request, id: int):
    ...
    # One walk for all of it: the restarts, their streams, and every stream's
    # size, however many restarts and containers there are.
    ...
    stats_by_restart: Dict[int, Dict[str, ModelInstanceLogStreamStats]] = {}
    # A run that failed before its container started has only its main log,
    # which is still the workload's stream and still worth measuring.
    restart_counts = {extract_restart_count(f) for f in files}
    for restart_count in restart_counts | set(container_names_by_restart):
        measured = {}
        for name in container_names_by_restart.get(restart_count) or ["default"]:
            paths = stream_log_files(entries, restart_count, name)
            measured[name] = await asyncio.to_thread(_stream_stats, paths)
        stats_by_restart[restart_count] = measured

    restarts = await asyncio.to_thread(
        restart_entries_from_main_log_files,
        files,
        container_names_by_restart,
        stats_by_restart,
    )
...
async def serve_log_line_range(
    ...
    Returns:
        ...
        X-Log-Offset, X-Log-Line-Count and X-Log-Total-Lines.
