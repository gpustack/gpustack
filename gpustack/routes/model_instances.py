        ...
        async def proxy_stream():
            try:
                async with client.get(model_instance_log_url, timeout=timeout) as resp:
                    if resp.status != 200:
                        error_response = OpenAIAPIException(
                            error=OpenAIAPIError(
                                message=f"Error fetching serving logs: {resp.reason}",
                                code=resp.status,
                                type="Error",
                            )
                        )
                        yield error_response.model_dump_json(), {}, resp.status
                ...
                yield error_response.model_dump_json(), {}, status.HTTP_500_INTERNAL_SERVER_ERRO
