...
def _is_self_user_filter(user: User, items) -> bool:
    """self scope 下仅当过滤的是调用者自己的 user_id 才放行。

    普通用户（effective_scope == self）按 user 过滤默认被拒（隐私），
    但若过滤项全部等于调用者自身的 user_id，则等价于 self 视图，允许通过。
    """
    if not items:
        return False
    for item in items:
        uid = item.identity.current.user_id if item.identity.current else None
...
def _check_permission(user: User, ctx, request, effective_scope: str) -> None:
    ...
    self scope 下仍允许“只过滤自己”的 user 过滤——它等价于 self 视图，
    不会泄露其它用户的数据。
    """
    ...
    if effective_scope == USAGE_SCOPE_SELF:
        ...
        if request.filters.users and not _is_self_user_filter(
            user, request.filters.users
        ):
    ...
    *,
    ...
    for group_by, items in [
        ...
        if not items:
            continue
        if (
            group_by == USAGE_GROUP_BY_USER
            and scope == USAGE_SCOPE_SELF
            and not _is_self_user_filter(user, filters.users)
        ):
            raise ForbiddenException(message="No permission to filter by user")
