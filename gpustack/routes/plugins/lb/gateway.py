...
- The **context role** (AUTHN/795) keeps the CR name
...
- The **finisher role** (AUTHN/700) is the new ``gpustack-lb`` CR; its
...
# Filter-chain positions from the plugin's deployment example; the
# capability plugins run between 795 and 700.
LB_CONTEXT_PHASE = "AUTHN"
LB_CONTEXT_PRIORITY = 795
LB_FINISHER_PHASE = "AUTHN"
LB_FINISHER_PRIORITY = 700
...
def lb_gateway_entries(cfg: Config) -> List[RouteGatewayEntry]:
    try:
        ...
        # exist until the policy table is written anyway.
        logger.error(
...
def redis_registry_from_url(
    ...
    if parsed.scheme != "redis" or not parsed.hostname:
        logger.error(
            "Invalid redis_url (want redis://[[user]:[pass]@]host"
            "[:port][/db]); staying on per-process shared data."
        )
        return None
    ...
    if registry_type == "static":
        ...
        # Envoy cluster address.
        domain = f"{parsed.hostname}:{parsed.port}" if parsed.port else parsed.hostname
    else:
