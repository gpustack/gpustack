...
class VersionConfig(BaseModel):
    ...
    Attributes:
        ...
        build_in_frameworks: Only build-in backend will return this field, sourced from gpustack-runner configuration. (Optional)
    ...
    build_in_frameworks: Optional[List[str]] = Field(None)
...
class InferenceBackendListItem(BaseModel):
    ...
    is_build_in: Optional[bool] = Field(None)
...
class InferenceBackendPublic(InferenceBackendBase):
    ...
    build_in_version_configs: Optional[Dict[str, VersionConfig]] = {}


InferenceBackendsPublic = PaginatedList[InferenceBackendPublic]


# build-in backend configurations
def get_build_in_backend() -> List[InferenceBackend]:
    return [
        InferenceBackend(backend_name=BackendEnum.VLLM, is_build_in=True),
        InferenceBackend(backend_name=BackendEnum.ASCEND_MINDIE, is_build_in=True),
        InferenceBackend(backend_name=BackendEnum.CUSTOM, is_build_in=True),
    ]


def get_build_in_backend_show_name(backend_name):
    name_map = {
