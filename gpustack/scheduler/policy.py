...
class ResourceFitPolicy:
    ...
    async def _filterOne(
        ...
        if is_unified_memory:
            ...
            for gpu_index in allocatable.gpu_memory:
                ...
                if index != -1 and allocatable.memory > self._estimate.memory[index].nonUMA.ram:
                    candidate = ModelInstanceScheduleCandidate(
                        worker=worker,
                        gpu_index=gpu_index,
                        computed_resource_claim=ComputedResourceClaim(
    ...
    async def _get_worker_allocatable_resource(self, worker: Worker) -> Allocatable:
        ...
        allocatable.memory = (worker.status.memory.total
                              - self._system_reserved.memory - allocated.memory)
        ...
        for gpu_index, gpu in enumerate(worker.status.gpu):
            ...
            if gpu_index == 0 and not worker.status.memory.is_unified_memory:
                allocatable.gpu_memory[gpu_index] = allocatable.gpu_memory.get(
                    gpu_index) - self._system_reserved.gpu_memory
