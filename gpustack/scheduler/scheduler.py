...
class Scheduler:
    ...
    async def _fan_out_dp_group(
        ...
        for placement, sibling in zip(node_placements, siblings):
            ...
            if model.extended_kv_cache and model.extended_kv_cache.is_shared():
                # Each DP node is placed on a single worker, so resolve the
                # shared-cache snapshot per node against its own worker
                # (spans_workers stays False: no node carries subordinates).
                sibling.cache_config = await resolve_instance_cache_config_safe(
                    session,
                    model,
                    worker=worker,
                    spans_workers=False,
