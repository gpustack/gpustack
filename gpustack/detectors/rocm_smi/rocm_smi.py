    ...
    def decode_rocm_smi(self, result) -> GPUDevicesInfo:
        ...
        for key in keys:
            ...
            if is_hygon:
                ...
                memory_total = int(info.get("vram Total Memory (MiB)", 0)) * 1024 * 1024
                memory_used = (
                    int(info.get("vram Total Used Memory (MiB)", 0)) * 1024 * 1024
                )
                utilization_gpu = float(info.get("DCU use (%)", 0))
                temperature_gpu = float(info.get("Temperature (Sensor mem) (C)", 0))
