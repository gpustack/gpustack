...
class RocmSMI(GPUDetector):
    ...
    def gather_gpu_info(self) -> GPUDevicesInfo:
        ...
        for device in rocm_smi_devices:
            rocminfo = rocminfo_devices.get(device.name)
            if rocminfo:
                device.name = rocminfo.get("Marketing Name")
                device.core.total = int(rocminfo.get("Compute Unit", 0))
