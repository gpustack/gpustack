...
def stall_error_sse(message: str = STALL_ERROR_MESSAGE) -> bytes:
    ...
    """
    # Leading blank lines terminate any partial frame the last chunk left
    # behind: chunks are relayed raw, so a stall can land mid-line, and the
    # error event must be a frame of its own rather than a continuation.
    return b"\n\ndata: " + stall_error_json(message) + b"\n\ndata: [DONE]\n\n"
