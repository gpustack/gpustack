...
def derive_dp_node_count(
    ...
    if load_balance_mode == "external":
        # external-LB pins exactly one DP rank per node (dpl == 1). Reject a
        # contradicting dpl here rather than let cal_multinode_topology fail on
        # the worker: the caller can still be told which flag to fix.
        if dpl is not None and dpl != 1:
            raise ValueError(
                f"vLLM external load-balance requires --data-parallel-size-local 1 "
                f"(got {dpl}); each node serves exactly one DP rank. Use "
                "--data-parallel-hybrid-lb to pack several ranks onto a node."
            )
        return dp
