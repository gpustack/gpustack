...
def has_manual_distributed_params(backend_parameters: Optional[List[str]]) -> bool:
    """Whether backend_parameters hand-wire a DP cluster (explicit
    ``--data-parallel-address`` / ``--node-rank`` / rank), signaling the user
    manages distribution themselves rather than letting GPUStack orchestrate it.
    """
    return any(
        find_parameter(backend_parameters, [param]) is not None
