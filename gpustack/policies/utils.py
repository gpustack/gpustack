...
from gpustack.utils.command import parse_bool_env
...
def manual_distributed_from_env(env: Optional[Dict[str, str]]) -> bool:
    """Whether ``env`` opts into manual distributed deployment
    (GPUSTACK_MANUAL_DISTRIBUTED): skip the GPU-count check and per-model gateway
    registration; the user wires the DP cluster and LB externally. Takes a raw
    env dict so callers with only the merged/effective env (e.g. model update)
    can reuse the same rule.
    """
    if not env:
        return False
    return parse_bool_env(env.get("GPUSTACK_MANUAL_DISTRIBUTED"))
...
def should_skip_gpu_count_check(model: Model) -> bool:
    ...
    if not model.env:
        return False
    return parse_bool_env(model.env.get("GPUSTACK_SKIP_GPU_COUNT_CHECK"))
