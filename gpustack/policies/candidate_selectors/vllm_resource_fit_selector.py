...
class VLLMResourceFitSelector(ScheduleCandidatesSelector):
    ...
    def _append_distributed_mode_message(self, messages):
        if (
    ...
    def _build_non_llm_message(self) -> str:
        # Non-LLM models
        default_message = f"- The model requires approximately {byte_to_gib(self._vram_claim)} GiB of VRAM.\n"
        if self._largest_multi_gpu_vram > 0:
            default_message += f"- The largest available worker has {byte_to_gib(self._largest_multi_gpu_vram)} GiB of VRAM.\n"
        elif self._largest_single_gpu_vram > 0:
            default_message += f"- The available GPU has {byte_to_gib(self._largest_single_gpu_vram)} GiB of VRAM.\n"
    ...
    def _build_gpu_message(self):
        ...
        if self._selected_gpu_workers and self._largest_multi_gpu_vram > 0:
            ...
            if (
                ...
                default_message += f"- The largest available worker has {byte_to_gib(self._largest_multi_gpu_vram)} GiB allocatable VRAM, {self._largest_multi_gpu_utilization_satisfied_count}/{self._largest_multi_gpu_total} of GPUs meet the VRAM utilization ratio, providing {self._cal_effective_vram():.2f} GiB of allocatable VRAM.\n"
            ...
            for worker_name, gpu_indexes in self._unsatisfied_gpu_messages.items():
                ...
                if log_worker_cnt > 2:
                    default_message += f"- Other {log_worker_cnt - 2} workers fail to meet, please check.\n"
                    break
                else:
                    default_message += f"- Worker {worker_name} GPU indexes {show_gpu_list} fails to meet the {(self._gpu_memory_utilization * 100):.2f}% allocatable VRAM ratio.\n"
                log_worker_cnt += 1
            ...
            default_message += f"- The current available GPU only has {byte_to_gib(self._largest_single_gpu_vram)} GiB allocatable VRAM ({(self._largest_single_gpu_vram_utilization * 100):.2f}%).\n"
    ...
    def _set_messages(self):
        ...
        if self._gpu_memory_utilization == 0:
            messages.append(self._build_non_llm_message())
        else:
            messages.append(self._build_gpu_message())
