...
def get_local_model_weight_size(local_path: str) -> int:
    """
    Get the size of the model weights from local path.
    """
    weight_file_extensions = (".safetensors", ".bin", ".pt", ".pth", ".gguf")
    total_size = 0

    if os.path.isfile(local_path):
        if local_path.endswith(weight_file_extensions):
            return os.path.getsize(local_path)
        return 0

    if os.path.isdir(local_path):
        for file in os.listdir(local_path):
            if file.endswith(weight_file_extensions):
