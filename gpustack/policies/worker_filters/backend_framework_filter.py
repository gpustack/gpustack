...
class BackendFrameworkFilter(WorkerFilter):
    ...
    def _get_backend_supported_frameworks(self, backend_name: str) -> List[str]:
            ...
            with Session(engine) as session:
                ...
                if backend and backend.version_configs and backend.version_configs.root:
                    for version_config in backend.version_configs.root.values():
                        ...
                        # For built-in backends, use build_in_frameworks
                        elif version_config.build_in_frameworks:
                            supported_frameworks.extend(
                                version_config.build_in_frameworks
                            )
        ...
        if backend_name in get_build_in_backend():
