...
class EventCollector:
    ...
    def format_events(self, filtered_level: List[EventLevelEnum]) -> List[str]:
        """Format filtered events into a string."""

        filtered_events = [
            event for event in self.events if event.level in filtered_level
        ]
        grouped_events = {event.action: event for event in filtered_events}

        formatted_events = []
        for action, event in grouped_events.items():
            formatted_event = (
                f"[Action: {event.action}]\n"
                f"Level: {event.level}, Message: {event.message}"
            )
