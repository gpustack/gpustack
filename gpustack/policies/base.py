...
@dataclass
class MemberResourceClaim:
    """What one member of a deployment costs, as a selector priced it.

    Read off the selector that has just run rather than computed again, so a
    refusal that quotes a figure and a breakdown that shows one cannot
    disagree about the same member. The group refusal path is the only caller:
    with no placement to read a claim off, this is the only honest source of
    "one prefill member wants 57.6 GiB".
    """
