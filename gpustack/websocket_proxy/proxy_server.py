...
class StallPolicy:
    ...
    error_sse: bytes = (
        b'\n\ndata: {"error":{"message":"upstream stalled"}}\n\ndata: [DONE]\n\n'
    )
...
async def wait_for_complete_response(  # noqa: C901
    ...
    def read_timeout() -> Optional[float]:
        ...
        if remaining <= 0:
            # A zero or negative budget must not be handed to the reader: some
            # readers treat it as "no timeout" and block indefinitely.
            raise asyncio.TimeoutError("Proxy total timeout exceeded")
