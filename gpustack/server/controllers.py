...
class ModelInstanceController:
    ...
    async def _reconcile(self, event: Event):  # noqa: C901
        """
        Reconcile the model.
        """
...
async def sync_replicas(session: AsyncSession, model: Model):
    ...
    if is_dp_per_instance:
        # A model that only just became DP-node-per-instance (or whose rows
        # predate the dp_rank column) still has instances carrying NULL, which
        # excludes them from the group: the scheduler won't fan them out and
        # the worker won't treat them as DP nodes. Backfill before handing out
        # ranks below, so new instances don't claim one they already hold.
        for instance in instances:
            if instance.dp_rank is not None:
                continue
            dp_rank = next(
            ...
            if dp_rank is None:
                # Scale-down pending: more instances than ranks. Whatever is
                # left over is about to be deleted below.
                break
            instance.dp_rank = dp_rank
