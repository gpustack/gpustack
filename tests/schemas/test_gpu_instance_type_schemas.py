...
def test_aggregated_public_sliced_detail_survives_passthrough():
    ...
    payload = {
        ...
        "status": {
            ...
            "onceMaxRequest": {"accelerator": "4", "acceleratorPartitioned": "1"},
            "remaining": {"accelerator": "16", "acceleratorPartitioned": "7"},
            "tiers": [
                {
    ...
    assert status["onceMaxRequest"]["acceleratorPartitioned"] == "1"
    assert status["remaining"]["acceleratorPartitioned"] == "7"
    assert status["tiers"][0]["acceleratorSlicedDetail"] == sliced
    candidate = status["tiers"][0]["candidates"][0]
    assert candidate["acceleratorSlicedDetail"] == sliced
    assert candidate["acceleratorPartitioned"] == {
        "onceMaxRequest": "1",
        "remaining": "7",
        "capacity": "14",
    }
