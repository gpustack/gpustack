...
def _distributed_env(shape, executor_backend="mp", with_deployment_metadata=True):
    ...
    instance: [http, dp_rpc, master, VLLM_PORT, connecting]. dp_rank is pinned
    to None: a bare MagicMock answers every attribute, which would send this
    plain instance down the DP-node-per-instance path.
    """
    server = object.__new__(VLLMServer)
    ...
    server._model_instance = MagicMock(
        ports=[40000, 40001, 40002, 40042, 40063], dp_rank=None
    )
...
def test_dp_member_without_coordinator_raises():
    """A member that can't resolve rank 0 fails loudly. Falling back to its own
    record would dial its own VLLM_PORT as the DP RPC port, and reading the
    master port off a 2-port member would run off the end of the list. The ray
    path resolves the same way, through _dp_port_source."""
    server = _dp_server(1, _dp_siblings(3))
    server._dp_siblings_cache = [s for s in _dp_siblings(3) if s.dp_rank != 0]

    with pytest.raises(ValueError, match="rank-0 coordinator"):
        _dp_mp_args(server, 1)
