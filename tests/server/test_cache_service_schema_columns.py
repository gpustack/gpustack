...
The cache_services migration creates ``state`` as VARCHAR. These tests
pin the ORM columns to plain strings so PostgreSQL never renders
native-enum casts (``$1::cacheservicestateenum``) that the database has
no type for, and so values are stored as the enum values, not the enum
member names.
"""
...
def test_state_columns_are_plain_strings():
    for column in (
