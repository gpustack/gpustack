...
async def test_soft_deleting_a_key_stops_it_authenticating(session, monkeypatch):
    ...
    Nothing drops the cache entry by hand here. ``ApiKey.delete`` owns that, so
    a caller that deletes the row without going through ``APIKeyService`` still
    converges instead of leaving the credential live for a cache TTL."""
...
@pytest.mark.asyncio
async def test_deleting_a_key_drops_its_cached_lookup(session):
    """The cache is warmed first, so this fails on the path authentication
    actually takes rather than on the query: a hard delete that left the entry
    behind would keep the credential resolving for a full TTL."""
    service = APIKeyService(session)
    assert await service.get_by_access_key(LIVE_ACCESS_KEY) is not None

    row = await ApiKey.one_by_id(session, 1)
    await row.delete(session)

    assert await service.get_by_access_key(LIVE_ACCESS_KEY) is None
