...
@pytest.mark.parametrize("reported", [2**31, 2**63, -1])
@pytest.mark.asyncio
async def test_an_out_of_range_ttft_is_stored_as_absent(session, caplog, reported):
    """``ttft_ms`` is a 32-bit ``Integer`` on every supported database, and the
    rollup and audit rows share one commit -- so a value no column can hold
    costs the whole flush window's usage, not one field, and keeps costing it
    while the batch is re-buffered. A duration below zero is a clock that moved
    backwards, which ``None`` describes and a negative number does not."""
    with caplog.at_level(logging.WARNING):
        await store_usage_metrics([], [_metric(ttft_ms=reported)])

    (row,) = await _details(session)
    assert row.ttft_ms is None
    # The rest of the row lands: the bound costs the TTFT, not the audit
    # record or every report queued behind it.
    assert row.prompt_token_count == 10
    assert "outside the [0, 2147483648) the column holds" in caplog.text


@pytest.mark.asyncio
async def test_a_ttft_at_the_bounds_is_kept(session):
    """The bounds are the column's range, not a margin inside it."""
    await store_usage_metrics(
        [], [_metric(ttft_ms=0), _metric(ttft_ms=2**31 - 1, request_id="second")]
    )

    assert {row.ttft_ms for row in await _details(session)} == {0, 2**31 - 1}
