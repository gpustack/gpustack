...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A dpl that agrees with that (1) is redundant but not a contradiction.
        (
            [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # external-LB is one rank per node, so a dpl > 1 contradicts it. Caught
        # here, not left for cal_multinode_topology to fail on the worker.
        [
            '--data-parallel-external-lb',
            '--data-parallel-size=4',
            '--data-parallel-size-local=2',
