...
| Field | Value |
|-------|-------|
| **Server URL** | `http://host.docker.internal:9090/v1-openai` <br>(for macOS/Windows)<br> or `http://<host_ip>:9090/v1-openai` (for Linux) |
...
```bash
docker exec -it <dify-container-name> curl http://host.docker.internal:9090/v1-openai/models
```
...
- On **Linux**, use the **real IP address** of the host machine (e.g., `192.168.1.100`).
...
| Scenario | Server URL |
|----------|------------|
| Host machine with pip-installed GPUStack <br>(macOS / Windows) | `http://host.docker.internal:9090/v1-openai` |
| Host machine with pip-installed GPUStack <br>(Linux) | `http://<host-ip>:9090/v1-openai` |
