...
| `--disable-worker`                  | (empty)                                | (DEPRECATED) Disable built-in worker. New installations will not have embedded worker by default. Use '--enable-worker' to enable embedded worker if needed. If neither flag is set, for backward compatibility, the embedded worker will be enabled based on presence of worker-related files in the data directory.                                                                                                                                                               |
| `--enable-worker`                   | `False`                                | Enable embedded worker. Valid for GPUStack server only.                                                                                                                                  |
