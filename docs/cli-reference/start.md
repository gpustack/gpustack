...
| `--forwarded-allow-ips` value                    | `127.0.0.1`                                      | Comma-separated peers whose `X-Forwarded-For` and `X-Forwarded-Proto` headers are trusted, or `*` to trust any. A reverse proxy reaching this port over the network must be listed here, otherwise the server cannot tell that TLS was terminated in front of it and will not mark session cookies `Secure`. |
