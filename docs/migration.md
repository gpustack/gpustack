...
!!! note
    ...
    Since v2.0.0, GPUStack officially supports only Linux. If you are using Windows or macOS, please move your data directory to a Linux system to perform the migration.

    On Windows and macOS, GPUStack Server (with the --disable-worker option) can still be run using Docker Desktop.
...
!!! note
    ...
    The support for GPU vendors has changed since version v2.0.0. Please refer to [Accelerator Runtime Requirements](./installation/requirements.md#accelerator-runtime-requirements).
...
!!! note
    ...
    Since v2.0.1, the environment variable `GPUSTACK_MIGRATION_DATA_DIR` introduced in v2.0.0 is deprecated. While setting it will still trigger database migration, its value will be ignored. It is recommended to use `GPUSTACK_DATA_MIGRATION=true` to indicate that the server requires database migration.
...
#### Migrate the Built-in Worker

The built-in worker will still be enabled after migration to v2.0.1. But if you are running a fresh install GPUStack server with v2.0.1, the built-in worker won't be enabled by default and it requires `--enable-worker` to enable it. You can check [Server Options](./cli-reference/start.md#server-options) for more details.

If the GPUStack server is running on the host without GPUs, You can use one of the following methods to perform the migration:

- If there are models running on built-in worker, Following [this document](#migration-from-llama-box) to change the models running with llama-cpp server.
- Remove the built-in worker and make it as a **Server Only** node with following start command. It's recommended to run on docker bridge network without built-in worker to reduce port confliction. It is safe to delete the built-in worker from the UI after the server has started successfully.
...
The following table lists supported GPU architectures and their corresponding runtime values:
...
| AMD GPU       | amd      |
| Ascend NPU    | ascend   |
| MThreads GPU  | mthreads |
| Iluvatar GPU  | iluvatar |
