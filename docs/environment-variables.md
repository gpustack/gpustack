...
| `GPUSTACK_MANUAL_DISTRIBUTED`                             | Manual distributed deployment for a vLLM data-parallel cluster you wire yourself: skips the GPU-count-vs-world-size check and this model's gateway registration (no upstream is registered, so manage the load balancer externally). Requires `enable_model_route=false`. Superset of `GPUSTACK_SKIP_GPU_COUNT_CHECK`.      | (empty) | Model          |
...
| `GPUSTACK_RUNTIME_DEPLOY_DEFAULT_CONTAINER_REGISTRY_USERNAME`      | Username for the default container registry. If GPUStack runner images are hosted in a container registry that requires authenticated access, set this variable to the access username at the startup of every GPUStack worker (or the all-in-one GPUStack server). In practice, it is recommended to configure the access username in the actual container runtime instead. | (empty) |
| `GPUSTACK_RUNTIME_DEPLOY_DEFAULT_CONTAINER_REGISTRY_PASSWORD`      | Password for the default container registry. If GPUStack runner images are hosted in a container registry that requires authenticated access, set this variable to the access password at the startup of every GPUStack worker (or the all-in-one GPUStack server). In practice, it is recommended to configure the access password in the actual container runtime instead. | (empty) |
| `GPUSTACK_RUNTIME_DEPLOY_DEFAULT_CONTAINER_NAMESPACE`              | Namespace for default runner container images. If GPUStack runner images are hosted outside the `gpustack` namespace, e.g. `gpustack-ai`, set this variable to `gpustack-ai`. For a multi-level namespace, e.g. `awesome.com/amazing-group/amazing-subgroup/gpustack-ai`, declare the registry with the parent namespace via `--system-default-container-registry` (or the worker `system_default_container_registry` config key, or the `GPUSTACK_SYSTEM_DEFAULT_CONTAINER_REGISTRY` environment variable), e.g. `--system-default-container-registry=awesome.com/amazing-group/amazing-subgroup`. | `gpustack` |
